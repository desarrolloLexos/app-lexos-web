...
export class UtilitarioService {
  ...
  public checkModificEncabezado(tarea: TareaOT, tareaClone: TareaOT): boolean {
    ...
    if (
      tarea.cliente === tareaClone.cliente && // Cliente
  ...
  public checkModificGeneral(tarea: TareaOT, tareaClone: TareaOT): boolean {
    if (
      tarea.latitud === tareaClone.latitud && // Latitud GPS
      ...
      tarea.detection_method_description ===
        tareaClone.detection_method_description && // Metodo deteccion de falla
  ...
  public checkExistsRecordGPS(tareaClone: TareaOT): boolean {
    if (
      tareaClone.latitud !== undefined &&
      tareaClone.latitud?.trim().length > 0 &&
      tareaClone.longitud !== undefined &&
      tareaClone.longitud?.trim().length > 0 &&
      tareaClone.direccion !== undefined &&
      tareaClone.direccion?.trim().length > 0
    ) {
      return true;
    }
    return false;
  ...
  public checkModificFirmas(tarea: TareaOT, tareaClone: TareaOT): boolean {
    if (
      tarea.observaciones === tareaClone.observaciones &&

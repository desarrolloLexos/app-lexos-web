import { Firestore, doc, getDoc, collection, getDocs, query, orderBy, where, addDoc, updateDoc, increment, collectionGroup, collectionData } from '@angular/fire/firestore';
...
export class ClientesService {
  ...
  async getLocalByCECO(uid: string, ceco: string){
    const q = query(collection(this.firestore, "clientes", uid, "locales")
                      .withConverter(LocalConverter),
                  where("ceco", "==", ceco),
                  orderBy("nombre", 'asc'));
  ...
  async setEquipo(uid: string,  equipo: Equipo){
    const equipoRef = await addDoc(
      collection(this.firestore, "clientes", uid, "equipos")
      .withConverter(EquipoConverter), {...equipo});
  ...
  async setLocal(uid: string,  local: Local){
    const localRef = await addDoc(
      collection(this.firestore, "clientes", uid, "locales")
      .withConverter(LocalConverter), {...local});
  ...
  getLocales(): Observable<Local[]> {
    const q = query(
      collectionGroup(this.firestore, 'locales').withConverter(
        LocalConverter
      ),
      orderBy("nombre", "asc")
    );
  ...
  getEquipos(): Observable<Equipo[]> {
    const q = query(
      collectionGroup(this.firestore, 'equipos').withConverter(
        EquipoConverter
      ),
      orderBy("nombre", "asc")
    );

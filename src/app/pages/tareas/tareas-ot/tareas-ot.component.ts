...
import { Component, ElementRef, Inject, OnDestroy, OnInit, ViewChild, ChangeDetectorRef, AfterContentChecked, AfterViewInit } from '@angular/core';
...
export class TareasOtComponent implements OnInit, OnDestroy, AfterContentChecked, AfterViewInit {
  ...
  masterToggle() {
    ...
    this.selection.select(...this.tareas.filter(row => row.id_status_work_order==7));
  }
  ...
  combinarOT(tecnico: Usuario){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.store.dispatch(actions.actualizarTareaOT({tareaOT: tareaPrincipal}));
        // --- Marcando tareas hijas
        this.selection.selected
        .filter( f => f.id_status_work_order === 7)
        .forEach( (tareaOT) => {
          if(tareaOT.id !== min_id){
            const tareaHija: TareaOT = {...tareaOT, 
                   id_parent_wo: min_id,
                    has_children: false,
                    personnel_description: tecnico.name,
                    id_assigned_user: tecnico.id_person,
            ...
            this.store.dispatch(actions.actualizarTareaOT({tareaOT: tareaHija}));
          }
        });

        this.store.pipe(select('tarea'), take(3), skip(2)).subscribe(
          (state) => {
  ...
  anularOT(){
    const cantidad = this.selection.selected
                     .filter(f => f.id_status_work_order !== 3)?.length;
    Swal.fire({
   ...
   */
    ...
    showCombinar(){
      return this.selection.hasValue() 
          ...
          && this.selection.selected.filter( f => f.id_status_work_order === 7)?.length > 1;
    }

    showAnular(){
      return this.selection.hasValue()
          && this.selection.selected.length > 0
          && this.selection.selected.filter( f => f.id_status_work_order !== 3)?.length > 0;
    }
  ...
  private filtrarOpciones(): TareaOT[] {
    ...
    if(this.fecha_inicio_start?.length > 0 && this.fecha_inicio_end?.length === 0){
      filterOT = filterOT.filter(t => t.initial_date.indexOf(this.fecha_inicio_start) !== -1);
    }

    if(this.fecha_inicio_start?.length > 0 && this.fecha_inicio_end?.length > 0){
      filterOT = filterOT.filter(t => t.initial_date >= this.fecha_inicio_start.concat("T00:00:00.000Z")
  ...
  eliminarFiltro(tipo: string, limpiarFiltro: boolean = false){
    this.filtros.forEach((filtro,index)=>{
     ...
     if(limpiarFiltro){
      switch(tipo){
        case 'cliente': { this.cliente = null; break; }
        case 'local': { this.local = null; break; }
        case 'idResponsable': { this.idResponsable = null; break; }
        case 'servicio': { this.servicio = null; break; }
        case 'estado': { this.estado = null; break; }
        case 'fecha_inicio': { 
  ...
  onChangeRange(event: MatDatepickerInputEvent<Date>){
    ...
    if(this.range){
      ...
      if(this.range.value?.start !== null){
        this.fecha_inicio_start = moment(this.range.value?.start).utc().format("YYYY-MM-DD").toString();
      }
      if(this.range.value?.end !== null){
        this.fecha_inicio_end = moment(this.range.value?.end).utc().format("YYYY-MM-DD").toString();        
      }

      if(this.fecha_inicio_start?.length > 0 && this.fecha_inicio_end?.length === 0){
        rango = moment(this.range.value?.start).format("DD/MM/YYYY").toString();
      }else if(this.fecha_inicio_start?.length > 0 && this.fecha_inicio_end?.length > 0){
        rango = moment(this.range.value?.start).format("DD/MM/YYYY").toString()
                .concat(" - ", 
...
export class TareaOtUser {
  ...
  constructor(
    ...
    private fb: UntypedFormBuilder,
        ...
        .pipe(
          ...
          map((state) => state.usuarios
          .filter(u => u.profiles_description === 'TECHNICAL' && u.active))
        )
        .subscribe((usuarios) => {
  ...
  onClick(): void {
      ...
      if(this.formResponsable.invalid) { return; }
       const {tecnico } =   this.formResponsable.getRawValue();
       this.responsable = this.responsables.filter( (user) => user.id_person === tecnico)[0];
